...
namespace ex18_winControlApp
{
    public partial class FrmMain : Form
    {
        ...
        // 글자체, 볼드, 이탤릭으로 변경하는 메소드
        void ChangeFont()
        {
            if (CboFonts.SelectedIndex < 0) // 아무것도 선택 안됨
                return; // 함수 빠져나감

            FontStyle style = FontStyle.Regular; // 일반 글자(볼드X, 이탤릭체X)로 초기화
        ...
        private void BtnAddChild_Click(object sender, EventArgs e)
        {
            ...
            {// 부모노드를 선택하지 않으면
                ...
                return; // 더이상 진행없이 메소드 종료
            }
            TrvDummy.SelectedNode.Nodes.Add(rand.Next().ToString());
            TrvDummy.SelectedNode.Expand(); // 트리노드 확장 (<-> Collaps)
