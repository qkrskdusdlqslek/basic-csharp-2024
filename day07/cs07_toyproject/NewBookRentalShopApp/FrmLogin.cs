...
namespace NewBookRentalShopApp
{
    public partial class FrmLogin : MetroForm
    {
        ...
        private void BtnLogin_Click(object sender, EventArgs e)
        {
            ...
            // DB연계
            IsLogin = LoginProcess(); // 로그인에 성공하면 True, 실패하면 False 리턴함
            if (IsLogin) this.Close(); // 현재 로그인창을 닫음
        }
        ...
        private bool LoginProcess()
        {
            ...
            * 5. Select SqlDataReader 또는 SqlDataSet 객체 불러서 사용
            ...
            */
            ...
            using (SqlConnection conn = new SqlConnection(Helper.Common.ConnString))
            {
                ...
                if (reader.Read())
                {
                    ...
                    chkPassword = reader["password"] != null ? reader["password"].ToString() : "-"; // 패스워드가 null이면 - 변경
        ...
        private void TxtUserId_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                TxtPassword.Focus(); // 패스워드로 포거스 이동
            }
        }
    }
}
