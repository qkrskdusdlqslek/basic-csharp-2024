namespace ex11_events
{
    ...
    delegate void EventHandler(string message); // 이벤트핸들러 대리자(어떠한 메소드를 대신 호출)

    class CustomNotifier // 미리 만들어져있다(고)
    {
        // 이벤트 등록, 이벤트라는 키워드를 쓰면 기본적으로 EventHandler 이름을 일반적으로 사용
        ...
        public void DoSomething(int number)
        {
            ...
            {   // 3, 6, 9 등의 상태가 되면 짝! 하는 이벤트를 발생시키겠다!
                ...
                // 이벤트핸들러 발생, 자신의 메소드가 아닌 외부에서 만들어진 메소드를 대신 실행
            }
        }
    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 1; i < 30; i++)
            {
                notifier.DoSomething(i); // 내장된 클래스의 어떠한 메소드를 호출
            }
           ...
           // MyDelegate callback; // 대리자
            // 메소드 이름이 존재X
            // 익명 메소드. 한번 사용이후 다시 호출할 필요가 없을 때 사용.
